...
public class TaskRepo implements GenericCRUD<Task> {
    ...
    private static final String CREATE_TASK = "INSERT INTO tasks (name, parent_id, owner_id, worker_id, " +
            "estimation_id, priority_id, status_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
    private static final String UPDATE_TASK = "UPDATE tasks SET name = ?, worker_id = ?, " +
            "priority_id =?, status_id = ? WHERE id = ?";
    private static final String DELETE_TASK = "DELETE FROM tasks WHERE id = ?";
    private static final String READ_TASK = "SELECT * FROM tasks t JOIN statuses s ON t.status_id=s.id " +
            "JOIN estimations e ON t.estimation_id=e.id " +
            "JOIN priorities p ON t.priority_id=p.id WHERE t.id=?;";
    private static final String READ_ALL_TASKS = "SELECT * FROM tasks t JOIN statuses s ON t.status_id=s.id " +
